...
scale=0.5,
atom/.style={circle,draw=black,fill=white!80!blue,minimum size=10},
dopant/.style={circle,draw=black,fill=white!80!red,minimum size=10}
]
\foreach \i in {1,...,6}
{
    \foreach \j in {1,...,4}
    {
        \node[atom] (\i\j) at (2*\i,2*\j) { $\tiny{\uparrow}$};  
    }
}
\node[dopant] ( a ) at (2,2) {$\tiny{\downarrow}$ };  
\node[dopant] ( b ) at (4,2) { $\tiny{\downarrow}$};  
\node[dopant] ( c ) at (8,4) {$\tiny{\downarrow}$ };  
\node[dopant] ( d ) at (12, 6) {$\tiny{\downarrow}$ };  
\node[dopant] ( a ) at (6,6) {$\tiny{\downarrow}$ };  
\node[dopant] ( a ) at (2, 8) {$\tiny{\downarrow}$ };  
\pause
\node[dopant] ( flip ) at (10,8) { $\tiny{\downarrow}$};  
\end{tikzpicture}
\end{figure} 
